    ...
    <>
      ...
      <section className="border-t border-white/10 bg-xo-obsidian py-24 md:py-32">
        <div className="xo-container grid grid-cols-1 gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            ...
            <Reveal delay={0.15}>
              <div className="mt-8 rounded-md border border-white/10 bg-xo-void p-6">
      ...
      <section className="border-t border-white/10 bg-xo-void py-24 md:py-32">
        <div className="xo-container">
          ...
          <div className="mt-14 grid grid-cols-1 gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 sm:grid-cols-2">
            {CHANGES.map((c, i) => (
              <Reveal key={c} delay={i * 0.05} className="bg-xo-void">
                <div className="flex items-center gap-4 p-8">
                  <span className="font-mono text-xs text-xo-teal">→</span>
                  <span className="text-[15px] text-white/70">{c}</span>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="relative overflow-hidden border-t border-white/10 bg-xo-obsidian py-24 md:py-32">
        ...
        <div className="xo-container relative">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
            ...
            <div className="font-display text-8xl font-semibold leading-none text-white/5">19</div>
          </div>

          <Reveal className="mt-14">
            <div className="grid grid-cols-1 overflow-hidden rounded-md border border-white/10 sm:grid-cols-2 lg:grid-cols-3">
              {TESTS.map((t, i) => (
                <div
                  key={t}
                  data-testid={`test-${i + 1}`}
                  className="group flex items-center gap-4 border-b border-r border-white/10 px-6 py-6 transition-colors duration-300 hover:bg-white/[0.03]"
                >
                  <span className="font-mono text-xs text-xo-teal tabular-nums">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="text-[15px] text-white/75 transition-colors group-hover:text-white">
                    {t}
                  </span>
                </div>
              ))}
              <div className="hidden border-b border-r border-white/10 lg:block" />
              <div className="hidden border-b border-r border-white/10 lg:block" />
            </div>
          </Reveal>
        </div>
          ...
          <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
            {DELEGATION.map((d, i) => (
              <Reveal key={d.title} delay={i * 0.08}>
                <div className="flex h-full flex-col rounded-md border border-white/10 bg-xo-obsidian p-8 transition-[transform,border-color] duration-500 hover:-translate-y-1 hover:border-xo-teal/40">
                  <div className="font-mono text-xs text-xo-teal">{String(i + 1).padStart(2, "0")}</div>
                  <h3 className="mt-4 font-display text-2xl text-white">{d.title}</h3>
                  <p className="mt-3 text-[15px] leading-relaxed text-white/50">{d.body}</p>
                  <div className="mt-auto pt-6 font-mono text-[11px] uppercase tracking-[0.15em] text-white/70">
                    {d.cert}
                  </div>
                </div>
              </Reveal>
        ...
        <div className="xo-container grid grid-cols-1 gap-16 lg:grid-cols-12">
          <div className="lg:col-span-6">
            ...
            <MaskTextInView
              ...
              as="span"
              className="font-display text-3xl font-medium leading-[1.06] tracking-tight text-white sm:text-4xl"
            />
            <Reveal delay={0.1}>
            ...
            <div className="overflow-hidden rounded-md border border-white/10">
              {SPECS.map((s, i) => (
                <Reveal key={s} delay={i * 0.03}>
                  <div className="flex items-center gap-4 border-b border-white/5 px-6 py-4 text-[14px] text-white/70 last:border-0">
