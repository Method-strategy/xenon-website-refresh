...
function MemberCard({ m, i, testid }) {
  return (
    <Reveal delay={(i % 4) * 0.05}>
      <div
        data-testid={testid}
        className="group flex h-full flex-col rounded-md border border-white/10 bg-xo-void p-6 transition-[transform,border-color] duration-500 hover:-translate-y-1 hover:border-xo-teal/40"
      >
        <div className="flex h-16 w-16 items-center justify-center rounded-full border border-white/10 bg-gradient-to-br from-xo-navy-deep/60 to-xo-obsidian font-display text-lg text-xo-teal">
          {initials(m.name)}
        </div>
        <h3 className="mt-6 font-display text-xl text-white">{m.name}</h3>
        <p className="mt-2 flex-1 text-[13px] leading-relaxed text-white/45">{m.title}</p>
        <div className="mt-5 flex items-center gap-3">
          {m.linkedin && (
            <a
              href={m.linkedin}
              aria-label={`${m.name} on LinkedIn`}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 text-white/50 transition-colors hover:border-xo-teal hover:text-xo-teal"
            >
...
export default function Team() {
  return (
    <>
      {/* Hero */}
      <section className="grain relative flex min-h-[60vh] items-end overflow-hidden bg-xo-obsidian pb-16 pt-40">
